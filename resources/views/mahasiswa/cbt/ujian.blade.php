...
@section('content')
    <div id="cbt-app" class="space-y-6">
        ...
        <div class="sticky top-16 z-10 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white px-5 py-4 shadow-sm ring-1 ring-gray-200">
            <div>
                ...
                <p class="text-xs text-gray-500">{{ $soalUrut->count() }} soal · Jawaban tersimpan otomatis</p>
            </div>
        ...
        <div class="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">
            ...
            <span>
                Jangan berpindah tab, keluar dari mode layar penuh, atau menyalin/menempel selama ujian —
                aktivitas ini tercatat sebagai pelanggaran dan dapat ditinjau panitia. Ujian akan otomatis
                dikumpulkan saat waktu habis.
            </span>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-4">
            ...
            <div class="order-2 lg:order-1">
                <x-ui-card>
                    <h2 class="text-xs font-semibold uppercase tracking-wide text-gray-500">Navigasi Soal</h2>
                    ...
                    <div class="mt-4 space-y-1.5 text-xs text-gray-500">
                        <div class="flex items-center gap-2"><span class="h-3 w-3 rounded bg-indigo-600"></span> Terjawab</div>
                        <div class="flex items-center gap-2"><span class="h-3 w-3 rounded bg-amber-400"></span> Ragu-ragu</div>
                        <div class="flex items-center gap-2"><span class="h-3 w-3 rounded border border-gray-300"></span> Belum dijawab</div>
                    </div>
                ...
                <x-ui-card>
                    @foreach ($soalUrut as $i => $s)
                        ...
                        <div class="cbt-soal-panel" data-cbt-panel="{{ $i }}" @if (! $loop->first) style="display:none" @endif>
                            <p class="text-xs font-medium uppercase tracking-wide text-gray-400">Soal {{ $i + 1 }} dari {{ $soalUrut->count() }}</p>
    ...
    <x-modal name="submit-cbt" :show="false" maxWidth="md">
        <div class="p-6">
            <h2 class="text-lg font-semibold text-gray-900">Kumpulkan Ujian?</h2>
            <p class="mt-2 text-sm text-gray-500">
                Setelah dikumpulkan, jawaban tidak dapat diubah lagi. Pastikan seluruh soal sudah Anda periksa.
            </p>
            <form method="POST" action="{{ route('mahasiswa.cbt.submit', $sesi) }}" class="mt-6 flex justify-end gap-3">
                @csrf
                <button type="button" class="rounded-lg px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100" onclick="window.dispatchEvent(new CustomEvent('close-modal', {detail: 'submit-cbt'}))">Batal</button>
                <button type="submit" class="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700">Ya, Kumpulkan</button>
            </form>
        </div>
    </x-modal>
