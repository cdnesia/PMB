<form method="POST" action="{{ route('mahasiswa.pendaftaran.bayar', $pendaftaran) }}" enctype="multipart/form-data" class="space-y-4">
    @csrf

    @if ($pendaftaran->promo && $pendaftaran->promo->isBerlaku() && $pendaftaran->promo->berlakuUntuk('pendaftaran'))
        <div class="flex items-start gap-2 rounded-lg bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
            ...
            <div>
                {{ __('pendaftaran.promo') }} <span class="font-semibold">{{ $pendaftaran->promo->kode }}</span> {{ __('pendaftaran.promo_active_registration') }}.
    ...
    <div class="rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-700">
        {{ __('pendaftaran.registration_fee') }}: <span class="font-semibold text-gray-900">Rp {{ number_format($pendaftaran->biayaPendaftaranAkhir(), 0, ',', '.') }}</span>
    </div>

    <div>
        <x-ui-label for="nominal" required>{{ __('pendaftaran.amount_paid') }}</x-ui-label>
        <div class="mt-2">
            <x-ui-input type="number" name="nominal" id="nominal" :value="old('nominal', $pendaftaran->biayaPendaftaranAkhir())" step="0.01" min="0" required placeholder="{{ __('pendaftaran.amount_example') }}" />
        </div>
