...
@section('content')
    <div class="mb-6">
        <a href="{{ route('mahasiswa.pendaftaran.index') }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-gray-500 transition hover:text-gray-900">
            <x-icon name="arrow-left" class="h-4 w-4" /> Kembali ke Pendaftaran Saya
        </a>
    </div>
    ...
    <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
        ...
        <div class="bg-gradient-to-r from-indigo-600 to-indigo-500 px-6 py-6 text-white">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <div class="text-xs font-medium uppercase tracking-wider text-indigo-200">Nomor Pendaftaran</div>
                    ...
                    <div class="mt-2 flex items-center gap-1.5 text-xs text-indigo-200">
                        ...
                        Terdaftar pada {{ $pendaftaran->created_at->format('d M Y, H:i') }}
                    </div>
                </div>
        ...
        <div class="p-6">
            ...
            @php
                ...
                $steps = [
                    ['label' => 'Pendaftaran', 'desc' => 'Formulir & dokumen terkirim'],
                    ['label' => 'Verifikasi Berkas', 'desc' => 'Pemeriksaan oleh panitia'],
                    ['label' => 'Seleksi & Pengumuman', 'desc' => 'Hasil seleksi diumumkan'],
                    ['label' => 'Daftar Ulang', 'desc' => 'Konfirmasi kelulusan'],
                    ['label' => 'Mahasiswa Baru', 'desc' => 'Resmi terdaftar'],
                ];
            ...
            <section aria-label="Progress pendaftaran">
                        ...
                        <li class="relative flex gap-4 pb-6 last:pb-0">
                            ...
                            <div class="pt-0.5">
                                <div class="text-sm font-semibold {{ $active ? 'text-gray-900' : ($done ? 'text-gray-700' : 'text-gray-400') }}">
                                    {{ $step['label'] }}
                                    @if ($active)
                                        <span class="ml-2 rounded-full bg-indigo-50 px-2 py-0.5 text-[11px] font-medium text-indigo-600">Saat ini</span>
                                    @endif
                                </div>
                ...
                <div class="mt-6 overflow-hidden rounded-xl border border-emerald-200">
                    <div class="bg-emerald-50 px-5 py-4">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="text-sm font-semibold text-emerald-900">Daftar Ulang & Pembayaran SPP</h3>
                        </div>
                        <p class="mt-1 text-xs text-emerald-700">Selamat! Anda dinyatakan <strong>lolos seleksi</strong>. Lengkapi daftar ulang dengan membayar uang kuliah (SPP).</p>
                    </div>
                        ...
                        @if ($pendaftaran->status === 'mahasiswa_baru' && $du?->status === 'lunas')
                            <div class="flex items-start gap-3 rounded-lg bg-emerald-50 px-4 py-3">
                                ...
                                <div class="text-sm text-emerald-800">
                                    ...
                                    Anda resmi menjadi <strong>Mahasiswa Baru</strong>.
                                </div>
                            </div>
                        @elseif ($du && $du->status === 'menunggu_verifikasi')
                            <div class="flex items-start gap-3 rounded-lg bg-amber-50 px-4 py-3">
                                ...
                                <div class="text-sm text-amber-800">
                                    <span class="font-semibold">Bukti pembayaran sedang diverifikasi.</span>
                                    <div class="mt-1 text-xs text-amber-700">Nominal: Rp {{ number_format($du->nominal, 0, ',', '.') }} · {{ $du->file_name }}</div>
                                </div>
                            </div>
                        @elseif ($du && $du->status === 'ditolak')
                            <div class="mb-4 flex items-start gap-3 rounded-lg bg-red-50 px-4 py-3">
                                ...
                                <div class="text-sm text-red-800">
                                    <span class="font-semibold">Bukti pembayaran ditolak.</span>
            ...
            <dl class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div class="rounded-lg bg-gray-50 px-4 py-3">
                    <dt class="text-xs font-medium uppercase tracking-wide text-gray-500">Gelombang</dt>
                ...
                <div class="rounded-lg bg-gray-50 px-4 py-3">
                    ...
                    @if ($pendaftaran->promo)
                        <dd class="mt-1 text-xs text-emerald-700">
                            Promo <span class="font-semibold">{{ $pendaftaran->promo->kode }}</span> — potongan {{ $pendaftaran->promo->labelPotongan() }}
                        </dd>
                    @endif
                </div>
            </dl>
            ...
            @if ($pendaftaran->pendaftar)
                ...
                <h3 class="mt-8 text-sm font-semibold text-gray-900">Data Diri</h3>
            ...
            <h3 class="mt-8 text-sm font-semibold text-gray-900">Pilihan Prodi & Kelas</h3>
            ...
            <h3 class="mt-8 text-sm font-semibold text-gray-900">Dokumen Persyaratan</h3>
            <div class="mt-3 space-y-3">
            ...
            @if ($pendaftaran->syaratJawaban->isNotEmpty())
                <h3 class="mt-8 text-sm font-semibold text-gray-900">Syarat Khusus Jalur</h3>
                <div class="mt-3 space-y-3">
